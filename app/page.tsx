...
const CompaniesPreview = () => {
	...
	const [selected, setSelected] = useState<
		(typeof POPULAR_COMPANIES)[number] | null
	>(null)
						...
						<Row gutter={[16, 16]}>
							{companies.map((company, index) => {
								const initials = company.name
									...
									.toUpperCase()
								const accent =
									company.accent ?? gradients[index % gradients.length]
				...
				<Space size={16} align='start'>
					...
					<Space direction='vertical' size={8} style={{ maxWidth: 420 }}>
						<Paragraph style={{ margin: 0 }}>
							Сравнивайте, что спрашивают в{' '}
							{selected?.name ?? 'компаниях'}, выбирайте формат интервью
							и отслеживайте, какие темы растут быстрее всего. Полные
							фильтры и подборки доступны после оформления Pro.
						</Paragraph>
						<Text type='secondary'>
							Демо-версия показывает только предпросмотр брендов без
							переходов внутрь.
						</Text>
					</Space>
				</Space>
		...
		<div style={{ background: '#f5f5f5' }}>
			<Space direction='vertical' size={64} style={{ width: '100%' }}>
				<div
					style={{
						background:
							'linear-gradient(135deg, #1e1b4b 0%, #312e81 35%, #1f2937 100%)',
						color: '#fff',
						padding: '80px 0'
					}}
				>
						...
						<Row gutter={[48, 32]} align='bottom'>
							<Col xs={24} lg={14}>
								...
								>
									...
									>
										Подготовься к собеседованию так, как будто ты уже
										внутри команды мечты
									</Title>
									<Paragraph
										style={{
									...
									>
										Частотные вопросы и реальные ответы по десяткам
										ролей. Показываем, что спрашивают прямо сейчас,
						...
						<Row align='middle' justify='space-between' gutter={[16, 16]}>
							<Col>
								<Space direction='vertical' size={8}>
									...
									<Text type='secondary'>
										Заходите внутрь конкретной компании и смотрите,
										какие темы спрашивают именно там. В демо — только
										предпросмотр брендов.
									</Text>
								</Space>
							</Col>
						</Row>
				...
				>
					<Space direction='vertical' size={24} style={{ width: '100%' }}>
						<Row align='middle' justify='space-between' gutter={[16, 16]}>
							<Col>
								<Space direction='vertical' size={8}>
									...
									<Text type='secondary'>
										Библиотека ответов кандидатов и разборов интервью:
										смотрите, как отвечают другие, и собирайте
										конспекты по таймкодам.
									</Text>
						...
						<Card style={{ borderRadius: 24 }} bodyStyle={{ padding: 0 }}>
							<Carousel dots>
								{[0, 1].map((page) => {
									const chunk = bundle.videoHighlights.slice(
										page * 3,
										page * 3 + 3
									)
						...
						<Col xs={24}>
							...
							>
								...
								<Paragraph style={{ margin: 0, color: '#595959' }}>
									Перед стартом убедись, что понимаешь условия
									использования и как мы бережно работаем с данными.
								</Paragraph>
								<Space direction='vertical' size={8}>
